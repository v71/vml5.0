...
namespace fa2040
{
	...
	class CleoCopter_2  
	{
		...
		//											// cleocopter belongs to see pathfinder for more info
			...
			void BoostController()
			{
				...
				float t0 = -4.0f;
				float t1 = -8.5f;
				float fx = t0 + BoostTime * (t1-t0);
				float bt = BoostTime*30.0f;
				...
				JetFlame1Overlay->AddAngleY(bt);
				JetFlame2Overlay->AddAngleY(bt);
			...
			void CreateObject(vml::models::ModelHandler* modelhandler, vml::overlays::OverlayHandler* overlayhandler)
			{
				...
				glm::vec3 startpos = glm::vec3(0, 0.4f/2, 11.5f/2)*Scale;
				...
				for ( size_t i=0; i< BulletOverlaysCount; ++i)
				{
					BulletOverlays[i] = overlayhandler->Add(vml::overlays::EffectOverlay::FAN,
															...
															startpos + glm::vec3(0, 0, BulletSpacing * (float)i), glm::vec3(270, 0, 90), glm::vec3(10, 5, 5)* Scale);
		...
		public:
			...
			void ComputeForces()
			{
				...
				bool leftShiftKeyPressed  = Keys[LEFT_SHIFT_KEY_PRESSED];
				bool rightShiftKeyPressed = Keys[RIGHT_SHIFT_KEY_PRESSED];

				// set initial accelleration to zero
				...
				if (!leftShiftKeyPressed)
				{
					...
					if (keypressed)
					{
						...
						Accelleration = (DirectionVector * ForceMagnitude * BoostFactorA + RbForceMagnitude * RbForceDirection) * InvMass;
					}
					else
					{
						// comptue decelleration due to viscous force

						Accelleration = RbForceMagnitude * RbForceDirection * InvMass;
					}
	...
	//		int GetNavMeshCellId() const { return NavMeshCellId; }
						 ...
						 /*  ,vml::handlers::Scene* scene*/ )
			{
				...
				BulletDt				= 2.5f   * Scale ;
				BulletSpacing			= 12.5f  * Scale ;
				Elongazion				= 150.0f * Scale ;
